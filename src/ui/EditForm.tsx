...
import useUpdateProduct from "../components/cms/useUpdateProduct";
...
export default function EditForm({
  ...
  const selectedProduct = products?.products.filter(
    (product) => product.id === Number(serachParams.get("id"))
  )?.[0];
  const { updateProduct, isUpdating } = useUpdateProduct();
  const [sizeArr, setSizeArr] = useState<string[]>([]);
  const [colorArr, setColorArr] = useState<string[]>([]);
  useEffect(() => {
    setColorArr(selectedProduct?.color.split(",") ?? []);
    setSizeArr(selectedProduct?.size.split(",") || []);
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    ...
    if (colorArr.length !== 0 && sizeArr.length !== 0) {
      ...
      updateProduct(product, {
        onSuccess: () => {
          if (setopenName) setopenName(null);
        },
      });
    } else {
  ...
  return (
    <div>
      <h1 className="text-3xl text-center fonst-bold ">
      ...
      >
          ...
          {colors.map((item) => (
            <label className="flex gap-1 text-gray-500 text-lg cursor-pointer">
              <input
                ...
                checked={colorArr.includes(item)}
                type="checkbox"
                name={item}
          ...
          {sizes.map((item) => (
            <label className="flex gap-1 text-gray-500 text-lg cursor-pointer">
              <input
                ...
                checked={sizeArr.includes(item)}
                type="checkbox"
                name={item}
                className="peer hidden "
              />
              <div
                className={`rounded-full px-2 py-1 peer-checked:bg-black peer-checked:text-white border `}
              >
                {item}
              </div>
            </label>
        ...
        {/* <label className="w-120 border border-gray-400 rounded-xl  pl-2 py-2 placeholder:text-gray-400 cursor-pointer">
          ...
          <span className="bg-black text-white py-2 px-4.5 rounded-xl">
            upload file
          </span>
        </label>
        ...
        <button
          className="text-white bg-black w-120 text xl py-2 rounded-xl"
          type="submit"
        >
          {isUpdating ? <Spinner /> : "OK"}
        </button>
      </form>
    </div>
  );
